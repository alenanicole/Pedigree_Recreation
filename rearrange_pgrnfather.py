...
def rearrange(tree, patientPerson):
    grandmotherId = (int)(globalVars.currentMaxID) + 1
    currentId = (int)(globalVars.currentMaxID) + 2 # Any new relatives to be added will start with the next available ID
    ...
    for x in range(6):
        ...
        # Paternal Grandfather -> Patient
        if((str)(relative.find('code').get('code'))== "PGRFTH"):
            # Since the new patient is the paternal grandfather, we must collect some data about him that will be
    ...
    for x in range(6, len(globalVars.relativesArray)):
        ...
        # Paternal Great-Grandmother -> mother
            ...
            # if the PGGRMTH's ID matches the original paternal grandfather mother's id, she will be the new mother
            if(id == PGRFTHMotherID):
        ...
        # Paternal Great-Grandfather -> father
            ...
            # if the PGGRFTH's ID matches the original paternal grandfather father's id, he will be the new father
            if(id == PGRFTHFatherID):
    ...
    # Finally, we will add the mother relative, father relative, grandmother relative (new patient's spouse),
